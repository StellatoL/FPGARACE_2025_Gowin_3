...
// localparam define
localparam QUICK_GATE_TIME         = 5;
localparam VERY_LOW_FREQ_GATE_TIME = 20; // 粗略/细测门控时间参考值
localparam LOW_FREQ_GATE_TIME      = 100;
localparam MEDIUM_FREQ_GATE_TIME   = 600;
localparam HIGH_FREQ_GATE_TIME     = 1500;
localparam VERY_HIGH_FREQ_ATE_TIME = 5000;
localparam LOW_DUTY_GATE_TIME      = 50;    // 占空比门控时间
localparam MEDIUM_DUTY_GATE_TIME   = 100;
localparam HIGH_DUTY_GATE_TIME     = 200;
localparam VERY_HIGH_DUTY_ATE_TIME = 500;

// 频率阈值 (Hz)
localparam FREQ_TH_3K  = 3_000;
localparam FREQ_TH_25K = 25_000;
...
always @(posedge clk_fs or negedge rst_n) begin
    if(!rst_n) begin
        state <= S_IDLE;
        freq_gate_time <= VERY_LOW_FREQ_GATE_TIME;
        ...
        case(state)
            S_IDLE: begin
                ...
                freq_gate_time <= QUICK_GATE_TIME;
                duty_gate_time <= LOW_DUTY_GATE_TIME;
                state <= S_COARSE_WAIT;
                digital_done <= 1'b0;
            end
            ...
            S_DECIDE: begin
                ...
                if(digital_frequency_temp <= FREQ_TH_3K) begin
                    freq_gate_time <= VERY_LOW_FREQ_GATE_TIME;
                    duty_gate_time <= LOW_DUTY_GATE_TIME;
                    // state <= S_FINE_WAIT;
                end else if(digital_frequency_temp <= FREQ_TH_25K) begin
                    ...
                    duty_gate_time <= VERY_HIGH_DUTY_ATE_TIME;
            ...
            S_DUTY_WAIT: begin
                if(measure_duty_done && !measure_duty_done_d) begin
                    digital_duty <= digital_duty_temp + 1'b1;
                    state <= S_OUTPUT;
                end
            end
