...
class FPGAMasterControl(QMainWindow):
    ...
    def send_serial_command_dac(self, data, is_binary=False):
        ...
        try:
            if is_binary:
                ...
                self.sent_data_display.append(f"[DAC] -> [HEX] {' '.join(f'{b:02X}' for b in data)}")
            else:
                self.serial_connection_dac.write(data.encode('ascii'))
                self.sent_data_display.append(f"[DAC] -> [ASCII] {data.strip()}")
        except serial.SerialException as e:
    ...
    def construct_and_send_frame(self):
        ...
        try:
            ...
            if frame: 
                self.send_serial_command_protocol(frame, is_binary=True)
    ...
    def send_serial_command_protocol(self, data, is_binary=False):
        ...
        try:
            if is_binary:
                ...
                self.sent_data_display.append(f"[PROTOCOL] -> [HEX] {' '.join(f'{b:02X}' for b in data)}")
            else:
                self.serial_connection_protocol.write(data.encode('ascii'))
                self.sent_data_display.append(f"[PROTOCOL] -> [ASCII] {data.strip()}")
        except serial.SerialException as e:
    ...
    def build_protocol_frame(self, protocol):
        ...
        if protocol == "PWM":
            ...
            # 合并启停状态(高四位)和选择字节(低四位)
            combined_byte = (enable_byte << 4) | (selection_byte & 0x0F)
            
            # 构建最终帧结构
            frame.append(combined_byte)  # 添加合并字节
            frame.extend(payload)        # 添加参数负载
            frame.extend(b'\r\n')        # 添加结束符
            
            return frame
        
        elif protocol == "I2C":
            selection_byte |= 0b001  # 设置I2C协议标识位
            ...
            # 创建组合字节：最高位为地址宽度标志，低三位为selection_byte
            combined_byte = (addr_width << 3) | (selection_byte & 0x07)
            
            # 构建payload
            payload = bytearray()
            payload.append(device_addr)  # 设备地址
            ...
            # 构建最终帧结构
            frame.append(combined_byte)  # 添加组合字节
            frame.extend(payload)        # 添加I2C数据
            frame.extend(b'\r\n')        # 添加结束符
            
            return frame

        elif protocol == "SPI":
            selection_byte |= 0b010  # 设置SPI协议标识位
            ...
            # 创建组合字节：最高位为tx_only，低三位为selection_byte
            combined_byte = (tx_only << 3) | (selection_byte & 0x07)
            
            # 构建最终帧结构
            frame.append(combined_byte)  # 添加组合字节
            frame.extend(payload)        # 添加SPI数据
            frame.extend(b'\r\n')        # 添加结束符
    ...
    def parse_received_frame(self, frame_data):
        ...
        try:
            ...
            # 根据协议类型解析数据
            payload = frame_data[3:-2]  # 去掉帧头和帧尾
            
            if protocol_type == 0b000:   # UART
                return f"[UART] 接收数据: {payload.hex(' ').upper()}"
                ...
                # 解析寄存器地址
                if addr_width:  # 16位地址
                    reg_addr = int.from_bytes(payload[1:3], 'big')
                    data_start = 3
                else:           # 8位地址
                    reg_addr = payload[1]
                    data_start = 2
                ...
                # 解析写入数据和读取长度
                write_data = payload[data_start:-1]  # 最后1字节是读取长度
                read_len = payload[-1]
                
                return (f"[I2C] 设备地址: 0x{device_addr:02X}, "
                        f"寄存器地址: 0x{reg_addr:0{4 if addr_width else 2}X}, "
                        f"写入数据: {write_data.hex(' ').upper()}, "
                        f"读取长度: {read_len}字节")
                ...
                # 解析写入数据长度（根据总长度和读取长度计算）
                read_len = int.from_bytes(payload[-1:], 'big')
                write_data = payload[:total_len-read_len]
                
                return (f"[SPI] 模式: {'TX Only' if tx_only else 'TX/RX'}, "
                        f"写入数据: {write_data.hex(' ').upper()}, "
                        f"读取长度: {read_len}字节")
            
            elif protocol_type == 0b011:  # PWM
                # 解析通道使能状态
                enable_byte = (extended_flags >> 4) & 0b1111
                channels = []
                ...
                for i in range(4):
                    ...
                    channels.append(f"通道{i+1}: {'启用' if enabled else '禁用'}, "
                                    f"PSC={psc}, ARR={arr}, Duty={duty}")
                
                return f"[PWM] 状态: {' | '.join(channels)}"
                ...
                flags = payload[4]
                data_len = payload[5]
                data = payload[6:6+data_len] if data_len > 0 else b''
