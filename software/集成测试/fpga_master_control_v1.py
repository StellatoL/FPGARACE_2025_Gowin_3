# -*- coding: utf-8 -*-
"""
fpga_unified_control_v3.0.py

功能:
这是一个集成化的FPGA上位机工具，合并了高性能示波器、波形绘制发送和高级协议调试功能。
1. [示波器] 通过共享内存从C++ UDP接收器高速读取ADC数据，使用pyqtgraph实时显示。
2. [波形绘制] 允许用户手绘波形，量化后通过串口发送。
3. [控制调试] 集成了DAC函数发生器和支持多种协议(PWM/I2C/SPI等)的高级协议调试器。
...
# 设置中文字体以解决 Matplotlib 的 UserWarning
try:
    import matplotlib.pyplot as plt
    plt.rcParams['font.sans-serif'] = ['SimHei']  # Or 'Microsoft YaHei'
    plt.rcParams['axes.unicode_minus'] = False
except Exception as e:
    print(f"警告: 无法设置中文字体，Matplotlib 预览图可能无法正确显示中文。错误: {e}")

# =============================================================================
# 辅助类 (来自 fpga_master_control_v1.1.py)
# =============================================================================
...
class FPGAMasterControl(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("FPGA 集成上位机 v3.0")
        self.setGeometry(100, 100, 1200, 900)

        # --- 初始化变量 ---
        self.shared_memory = None
        self.serial_connection_control = None # 用于控制和调试的串口实例
    ...
    # =============================================================================
    # 选项卡 1: 高速数字示波器 (来自 fpga_master_control.py)
    # =============================================================================
    def init_scope_tab(self):
        # ... 此部分代码与 fpga_master_control.py 完全相同，无需修改 ...
    ...
    def update_scope_plot(self):
        ...
        try:
            ...
            print(f"读取共享内存或更新绘图时出错: {e}")
            self.stop_scope()

    # =============================================================================
    # 选项卡 2: 波形绘制与发送 (来自 fpga_master_control_v1.1.py)
    # =============================================================================
    ...
    def init_control_tab(self):
        ...
        self.baud_combo_control.addItems(["9600", "19200", "57600", "115200"])
        ...
        self.protocol_combo.addItems(["PWM", "I2C", "SPI", "UART"])
        ...
        control_panel_layout.addLayout(left_panel, 2) # 比例 2
        control_panel_layout.addLayout(right_panel, 1) # 比例 1

        # --- 整体布局 ---
        main_layout.addWidget(serial_group)
        main_layout.addLayout(control_panel_layout)
        self.refresh_all_ports() # 初始化
    ...
    def init_protocol_pages(self):
        ...
        for i in range(1, 3): # 简化为2个通道
            psc = QLineEdit("1000"); arr = QLineEdit("255"); duty = QLineEdit("128")
            layout.addRow(f"--- 通道 {i} ---", None)
            layout.addRow(f"PSC Ch{i} (8b):", psc); layout.addRow(f"ARR Ch{i} (8b):", arr); layout.addRow(f"Duty Ch{i} (4b):", duty)
        ...
        self.i2c_device_addr = QLineEdit("68"); self.i2c_reg_addr = QLineEdit("01"); self.i2c_data_to_write = QLineEdit("AA BB"); self.i2c_read_len = QLineEdit("4")
        layout.addRow("寄存器地址宽度:", self.i2c_addr_width); layout.addRow("设备地址 (Hex):", self.i2c_device_addr); layout.addRow("寄存器地址 (Hex):", self.i2c_reg_addr); layout.addRow("写入数据 (Hex):", self.i2c_data_to_write); layout.addRow("读取长度(Bytes):", self.i2c_read_len)
    ...
    def connect_serial_control(self):
        port = self.ports_combo_control.currentData()
        baud = int(self.baud_combo_control.currentText())
        if not port: self.show_error("请选择有效的串口！"); return
        try:
            ...
            self.statusBar().showMessage(f"成功连接到 {port} @ {baud} bps", 5000)
        except serial.SerialException as e: self.show_error(f"无法打开串口 {port}: {e}")
    ...
    def send_serial_command_control(self, data, is_binary=False):
        ...
        try:
            if is_binary:
                ...
                if response:
                    self.received_data_display.append(f"<- [HEX] {' '.join(f'{b:02X}' for b in response)}")
            else:
                ...
                if response:
                    self.received_data_display.append(f"<- [ASCII] {response}")
    ...
    def send_dac_command(self):
        ...
        try:
            ...
            self.send_serial_command_control(command)
        except ValueError: self.show_error("频率和幅值必须是有效的数字！")
    ...
    def construct_and_send_frame(self):
        try:
            ...
            if frame:
                self.send_serial_command_control(frame, is_binary=True)
    ...
    def build_protocol_frame(self, protocol):
        # 协议帧格式: [0x20, 0x25, selection_byte, payload..., '\r', '\n']
        frame = bytearray([0x20, 0x25]); selection_byte = 0; payload = bytearray()
        if protocol == "PWM":
            selection_byte |= 0b011
            for i in range(len(self.pwm_inputs)):
                psc = int(self.pwm_inputs[i]['psc'].text())
                arr = int(self.pwm_inputs[i]['arr'].text())
                duty = int(self.pwm_inputs[i]['duty'].text())
                payload.extend(psc.to_bytes(8, 'big'))
                payload.extend(arr.to_bytes(8, 'big'))
                payload.extend(duty.to_bytes(4, 'big'))
        elif protocol == "I2C":
