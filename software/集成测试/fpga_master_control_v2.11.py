# -*- coding: utf-8 -*-
"""
fpga_unified_control_v3.5_full.py (已修复和优化) 改变了手绘波形发送
对应cpp_v1
...
主要修改 (编码助手 v3.7 - 低频锁定修复版):
1. [新增-滚动缓冲] 在示波器的数据处理中引入了一个大型滚动数据缓冲区(SCOPE_ROLLING_BUFFER_SIZE)。
2. [修复-低频锁定] 通过将新接收的数据块追加到滚动缓冲区，解决了因数据包过小而无法捕捉到低频信号（如100Hz正弦波）完整周期的问题，从而实现了对低频信号的稳定触发和锁定。
3. [优化-代码结构] 将滚动缓冲区的逻辑无缝集成到`update_scope_plot`函数中，原有的健壮触发函数`apply_trigger`无需修改即可在新缓冲区上正常工作。
"""
import sys
import mmap
import struct
...
class FPGAMasterControl(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("FPGA 集成上位机 v3.7 (低频锁定修复版)")
    ...
    # =============================================================================
    # 选项卡 1: 高速数字示波器 - 逻辑无变化，但现在作用于滚动缓冲区
    # =============================================================================
        ...
        try:
            ...
            with serial.Serial(port_name, DEFAULT_BAUD_RATE, timeout=2, write_timeout=2) as ser:
                for address in range(NUM_SAMPLES_SEND):
                    ...
                    # 波形选择(高4位)+频率选择(低4位)，若无可用数据则置0，自定义波形是5
                    frame.append(0x50)
